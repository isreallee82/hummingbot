...
class EvedexPerpetualWebSocketTests(IsolatedAsyncioWrapperTestCase):
    ...
    def test_place_open_order_rejects_when_close_order_is_in_flight(self):
        self.exchange.start_tracking_order(
            order_id="OID_CLOSE",
        ...
        close_order = self.exchange.in_flight_orders["OID_CLOSE"]
        close_order.current_state = OrderState.OPEN
        ...
        with self.assertRaisesRegex(
            ValueError,
            "Cannot place OPEN order for BTC-USDT while CLOSE order OID_CLOSE is still in flight.",
        ):
            self.async_run_with_timeout(
                self.exchange._place_order(
                    order_id="OID_OPEN",
    ...
    def test_place_open_order_allows_after_close_order_is_filled(self):
        self.exchange.start_tracking_order(
            order_id="OID_CLOSE_FILLED",
            exchange_order_id="EX_CLOSE_FILLED",
            trading_pair=self.trading_pair,
            trade_type=TradeType.BUY,
            price=Decimal("10"),
            amount=Decimal("3"),
        ...
        close_order = self.exchange.in_flight_orders["OID_CLOSE_FILLED"]
        close_order.current_state = OrderState.FILLED
        ...
        self.exchange.exchange_symbol_associated_to_pair = AsyncMock(return_value=self.ex_trading_pair)
        self.exchange.get_leverage = MagicMock(return_value=2)
        self.exchange._api_post = AsyncMock(return_value={"id": "ex_open_after_close", "createdAt": 456})
        self.exchange._auth = MagicMock()
        self.exchange._auth.wallet_address = "0xabc"
        self.exchange._auth.sign_market_order = MagicMock(return_value="0xsig")
        ...
        self.assertEqual(exchange_order_id, "ex_open_after_close")
        self.assertEqual(transact_time, 456)
