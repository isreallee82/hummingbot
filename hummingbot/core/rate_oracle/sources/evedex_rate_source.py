...
class EvedexRateSource(RateSourceBase):
    def __init__(self):
        super().__init__()
        self._exchange: Optional[EvedexExchange] = None
    ...
    async def get_prices(self, quote_token: Optional[str] = None) -> Dict[str, Decimal]:
        ...
        try:
            ...
            self.logger().exception(
                msg="Unexpected error while retrieving rates from EvedEx. Check the log file for more info.",
            )
        return results

    def _ensure_exchange(self):
        if self._exchange is None:
            self._exchange = self._build_evedex_connector_without_private_keys()

    @staticmethod
    def _build_evedex_connector_without_private_keys() -> 'EvedexExchange':
        from hummingbot.connector.exchange.evedex.evedex_exchange import EvedexExchange
