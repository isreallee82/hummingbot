...
class HyperliquidConfigMap(BaseConnectorConfigMap):
    ...
    @field_validator("hyperliquid_secret_key", mode="before")
    @classmethod
    ...
    def validate_key_matches_address(self):
        ...
        validate_key_address_pair(
            getattr(self, "hyperliquid_mode", "arb_wallet"),
            getattr(self, "use_vault", False),
            getattr(self, "hyperliquid_address", None),
            getattr(self, "hyperliquid_secret_key", None),
        )
        return self
...
class HyperliquidTestnetConfigMap(BaseConnectorConfigMap):
    ...
    @field_validator("hyperliquid_testnet_secret_key", mode="before")
    @classmethod
    def validate_secret_key(cls, value):
        """Reject malformed private keys at connect time (issue #7866)."""
        return validate_private_key_format(value)
    ...
    def validate_key_matches_address(self):
        """Ensure the private key derives to the supplied wallet address (#7866)."""
        validate_key_address_pair(
            getattr(self, "hyperliquid_testnet_mode", "arb_wallet"),
            getattr(self, "use_vault", False),
            getattr(self, "hyperliquid_testnet_address", None),
            getattr(self, "hyperliquid_testnet_secret_key", None),
        )
        return self
