...
class LPRebalancer2Config(ControllerConfigBase):
    """
    Configuration for LP Rebalancer 2 Controller.
    ...
    Key differences from LPRebalancer:
...
class LPRebalancer2(ControllerBase):
    """
    ...
    def __init__(self, config: LPRebalancer2Config, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
        self.config: LPRebalancer2Config = config
    ...
    def to_format_status(self) -> List[str]:
        ...
        header = f"| LP Rebalancer 2: {self.config.trading_pair} on {self.config.connector_name}"
